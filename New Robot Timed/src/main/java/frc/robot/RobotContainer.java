...
SetInternalMoveSpeed setInternalMoveSpeedTAS = new SetInternalMoveSpeed(shooter);
 FeederMotor feederMotor = new FeederMotor(shooter);
  ...
  Command taxiAndShootTwoBalls = (new ResetDriveTrainPosition(vroomVroom))
    .andThen(new OffTarmac(vroomVroom)).alongWith(new FeederMotor(shooter))
    .andThen(new Spin(vroomVroom)).andThen(new Targeting(vroomVroom, vision))
    .andThen(new SpinUpFlyWheel(shooter)).andThen(new DoNotMove(vroomVroom));
  ...
  JoystickButton angleOverRideButton = new JoystickButton(rightStick, Constants.ANGLED_OVERRIDE_BUTTON);
