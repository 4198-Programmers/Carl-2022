...
public class VertHooks extends SubsystemBase {
    private CANSparkMax verticalHooksLead = new CANSparkMax(Constants.VERT_HOOKS_OG_MOTOR_DEVICE_ID, MotorType.kBrushless);
    private CANSparkMax verticalHooksFollower = new CANSparkMax(Constants.VERT_HOOKS_FOLLOWER_MOTOR_DEVICE_ID, MotorType.kBrushless);
    ...
    public double getVerticalHookHeight() { //actual hook height not motor encoder
        return Maths.motorConversion((Math.abs(vertHookLeadEncoder.getPosition()) + Math.abs(vertHookFollowerEnc.getPosition()))/2);
    }
